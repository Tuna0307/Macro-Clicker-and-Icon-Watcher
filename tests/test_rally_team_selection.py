...
class RallyTeamSelectionTests(unittest.TestCase):
    ...
    def test_cleanup_abort_clears_flag_when_stop_interrupts_cleanup(self):
        joining_step = Step(name="Joining", enabled=True)
        attack_step = Step(
            name="Attack Confirm",
            enabled=True,
        ...
        engine = object.__new__(MacroEngine)
        engine.scenario = Scenario(
            name="Interrupted cleanup abort",
            steps=[joining_step, attack_step],
        )
        stop_event = type(
            "Stop",
            (),
            {
                "stopped": False,
                "is_set": lambda self: self.stopped,
            },
        )()
        ...
        self.assertTrue(engine._cycle())
        self.assertEqual(executed, ["select_rally_team", "set_step"])
        self.assertFalse(joining_step.enabled)
        self.assertTrue(attack_step.enabled)
        self.assertFalse(engine._cleanup_after_abort)
    ...
    def test_visible_team3_limit_controls_joining_prefilter(self):
        ...
        engine._best_scaled_template_match = lambda _frame, path: (
            (1.0 if path == joining_action.team1_busy_template_path else 0.0),
            (0, 0),
        )
    ...
    def test_availability_diagnostics_identify_the_limit_selector(self):
        selector_action = Action(
            type="select_rally_team",
            team1_max_level=65,
            team3_max_level=50,
        )
        joining_action = Action(
            type="click_matching_row",
            max_level=65,
        ...
        engine.scenario = Scenario(
            ...
            steps=[
                Step(name="Before", actions=[Action(type="wait")]),
                Step(
                    name="Dispatch",
                    actions=[Action(type="wait"), selector_action],
                ),
                Step(name="Joining", actions=[joining_action]),
            ],
        )
        engine._pending_rally_team_availability = None
        engine._last_rally_team_busy_state = None
        engine._last_rally_team_availability = {}
        engine.log = lambda _message: None
        engine._get_target_window_rect = lambda: (0, 0, 100, 100)
        engine._grab = lambda region: (
            np.zeros((region[3], region[2], 3), dtype=np.uint8),
            region[0],
            region[1],
        ...
        self.assertEqual(
            engine._last_rally_team_availability["level_limits_selector"],
            {
                "step_name": "Dispatch",
                "step_index": 1,
                "action_index": 1,
    ...
    def test_ordinary_row_ignores_pending_smart_availability_cap(self):
        action = Action(
            type="click_matching_row",
            match_condition_index=0,
            on_condition_index=1,
